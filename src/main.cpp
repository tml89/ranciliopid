...
void setup() {
    ...
    if (config.get<int>("hardware.leds.type") == 1) {  // 1=WS2812
...
void handleStandardLED() {
    ...
    if (machineState == kSensorError || machineState == kEmergencyStop) {
    ...
    bool tempReady = ((machineState == kPidNormal || machineState == kBrew) && (fabs(temperature - setpoint) < 1.0)) ||
...
void handleWS2812() {
    ...
    } else if (machineState == kSensorError || machineState == kEmergencyStop) {
    ...
    if (machineState == kBackflush) {
        ...
        double value = (double)currBrewTime / 1000.0 / TARGET_BREW_TIME;
        if (value > 1.0) value = 1.0;
        leds[STATUS_LED].setHue((uint8_t)(85 - (90 * value)));
    } else {
        bool ready = ((machineState == kPidNormal || machineState == kBrew) && (fabs(temperature - setpoint) < 1.0)) ||
                     (machineState == kSteam && temperature > steamSetpoint - 2);
        leds[STATUS_LED] = ready ? CRGB::Black : CRGB::White;
    }
